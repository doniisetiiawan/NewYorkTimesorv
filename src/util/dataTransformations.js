...
const getMultimediaUrlByFormat = (multimedia, format) => {
  ...
  const matchingFormat = multimedia.find(
...
export const reshapeNewsData = (news) => news.map(
  ({
    abstract,
    byline,
    geo_facet,
    ...
    imageUrl: getMultimediaUrlByFormat(
      multimedia,
      'thumbLarge',
    ),
...
export const filterNewsBySearchTerm = (
  ...
  return newsItems.filter(
    ({ description, author, title }) => description.toLowerCase().indexOf(searchTerm) > -1
